...
test('手动组列表按 section 分组排序，"其他"排在每个 section 末尾', () => {
  const multiRegion = {
    '代理选择标签': { name: '代理选择标签', type: 'Selector', now: '全部聚合/手动组', all: [] },
    '全部聚合/手动组': { name: '全部聚合/手动组', type: 'Selector', now: '节点A', all: ['节点A'] },
    '按地区/香港/手动组': { name: '按地区/香港/手动组', type: 'Selector', now: '节点B', all: ['节点B'] },
    '按地区/日本/手动组': { name: '按地区/日本/手动组', type: 'Selector', now: '节点C', all: ['节点C'] },
    '按地区/新加坡/手动组': { name: '按地区/新加坡/手动组', type: 'Selector', now: '节点D', all: ['节点D'] },
    '按地区/其他/手动组': { name: '按地区/其他/手动组', type: 'Selector', now: '节点E', all: ['节点E'] },
    '按地区/香港/自动组': { name: '按地区/香港/自动组', type: 'URLTest', now: '节点B', all: ['节点B'] },
    '按地区/日本/自动组': { name: '按地区/日本/自动组', type: 'URLTest', now: '节点C', all: ['节点C'] },
    '按地区/新加坡/自动组': { name: '按地区/新加坡/自动组', type: 'URLTest', now: '节点D', all: ['节点D'] },
    '按地区/其他/自动组': { name: '按地区/其他/自动组', type: 'URLTest', now: '节点E', all: ['节点E'] },
  };
  const model = buildProxyUiModel(multiRegion);
  const manualNames = model.manualSections.flatMap((section) => section.groups.map((g) => ({
    section: section.title,
    name: g.name,
  })));
  const otherGroups = manualNames.filter((g) => g.name.includes('其他'));
  const nonOtherGroups = manualNames.filter((g) => !g.name.includes('其他'));
  const maxNonOther = nonOtherGroups.length > 0
    ? Math.max(...nonOtherGroups.map((g) => manualNames.indexOf(g)))
    : -1;
  const minOther = otherGroups.length > 0
    ? Math.min(...otherGroups.map((g) => manualNames.indexOf(g)))
    : manualNames.length;
  assert.ok(otherGroups.length >= 1, '至少应有一个"其他"组');
  assert.ok(minOther > maxNonOther, '"其他" 组应在所有非其他组的后面');
});

test('模型给实际路由节点附带延时和语义状态', () => {
  const model = buildProxyUiModel(proxies, {
    delayCache: new Map([['香港-猫熊机场-V301', { delayMs: 850, status: 'good' }]]),
  });

  assert.deepEqual(model.routeNodeDelay, { delayMs: 850, status: 'good' });
});
...
test('HTML 移除自动组选择器重复小字但保留可访问名称', async () => {
  ...
  assert.match(html, /<div[^>]*id="auto-group-cards"[^>]*aria-label="自动组代理选择器"/);
});

test('自动组选中卡片视觉与手动节点选中态一致', async () => {
  const css = await readFile(new URL('../ui/styles.css', import.meta.url), 'utf8');
  const selectedBlock = cssBlock(css, '.auto-group-card.selected');
  assert.ok(selectedBlock, '必须提供 .auto-group-card.selected 规则');
  assert.match(selectedBlock, /border:\s*3px\s+solid\s+#f59e0b/, '选中边框应为 3px 粗 amber');
  assert.match(selectedBlock, /background:\s*linear-gradient\(\s*135deg,\s*#fef3c7,\s*#dbeafe\)/, '选中背景应为浅黄到浅蓝渐变');
  assert.match(selectedBlock, /color:\s*#111827/, '选中文字应为深色');
});
...
test('自动代理选择器标题 span 继承 section-heading 大字号', async () => {
  const css = await readFile(new URL('../ui/styles.css', import.meta.url), 'utf8');
  assert.match(css, /\.section-heading\s+span\s*\{[^}]*font-size:\s*inherit/s);
  assert.match(css, /\.section-heading\s+span\s*\{[^}]*font-weight:\s*inherit/s);
});

test('实际路由节点段包含延时徽章样式和语义色 class 支持', async () => {
  const css = await readFile(new URL('../ui/styles.css', import.meta.url), 'utf8');
  assert.match(css, /\.route-node-delay\s*\{/);
...
test('模板 mixed.json 中 代理选择标签默认全部聚合且保留 direct 支持 toggleProxy', async () => {
  ...
  assert.equal(selector.default, '全部聚合/自动组', '代理选择标签默认应为全部聚合/自动组');
  assert.equal(selector.outbounds[0], '全部聚合/自动组', '代理选择标签.outbounds[0] 应为全部聚合/自动组');
