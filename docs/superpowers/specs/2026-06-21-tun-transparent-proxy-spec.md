...
```json
{
  ...
  "outbounds": [
    ...
    // ── 按地区分组 ──
    { "type": "urltest", "tag": "按地区/香港/自动组", "outbounds": ["香港-{节点名}", ...], "interval": "20s" },
    { "type": "selector", "tag": "按地区/香港/手动组", "outbounds": ["香港-{节点名}", ...] },
    { "type": "urltest", "tag": "按地区/日本/自动组", "outbounds": ["日本-{节点名}", ...], "interval": "20s" },
    { "type": "selector", "tag": "按地区/日本/手动组", "outbounds": ["日本-{节点名}", ...] },
    { "type": "urltest", "tag": "按地区/新加坡/自动组", "outbounds": ["新加坡-{节点名}", ...], "interval": "20s" },
    { "type": "selector", "tag": "按地区/新加坡/手动组", "outbounds": ["新加坡-{节点名}", ...] },
    { "type": "urltest", "tag": "按地区/美国/自动组", "outbounds": ["美国-{节点名}", ...], "interval": "20s" },
    { "type": "selector", "tag": "按地区/美国/手动组", "outbounds": ["美国-{节点名}", ...] }
  ],
  "route": {
    "rules": [
...
- 节点命名规范（sub-store 输出时需确保 tag 包含前缀，`{节点名}` 来自订阅不做预定）：
  - `香港-机场A-{节点名}`
  - `日本-机场B-{节点名}`
- 分组脚本通过前缀解析节点归属：
  - 机场维度：取 tag 第二段
...
4. 实际路由标签显示：`按地区/香港/自动组 → 香港-{urltest选中的节点}`

**手动模式：**
1. Web UI → 直接打开任意手动组（如 `按地区/香港/手动组`）
2. 选一个具体节点（名称来自订阅）
3. `route.final` 更新为 `按地区/香港/手动组` + 在该组内选中该节点
4. 代理选择器反向更新为 `按地区/香港/手动组`
5. 实际路由标签显示：`按地区/香港/手动组 → {用户选的节点}`
