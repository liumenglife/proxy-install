...
test('独立代理 UI 显示顶部控制、只读路由和自动组列表', async ({ page, request }) => {
  ...
  await expect(page.getByText('自动组代理选择器')).toBeVisible();
  await expect(page.getByText('实际路由标签')).toBeVisible();
  await expect(page.getByTestId('route-label')).toContainText('→');
...
test('自动组只读说明显示静态提示文案', async ({ page }) => {
  await page.goto(`${frontendUrl}?_pw=${Date.now()}`, { waitUntil: 'networkidle' });

  await expect(page.getByTestId('readonly-auto-note')).toBeVisible();
  await expect(page.getByTestId('readonly-auto-note')).toHaveText('自动组不可手动选择，请去对应手动组选择');
});
...
test('自动组切换和手动组节点选择会刷新并保持代理选择标签一致', async ({ page, request }) => {
  ...
  await expect(page.getByTestId('route-label')).toContainText(automaticGroup);
  ...
  await expect.poll(async () => {
    ...
    return page.getByTestId('route-label').textContent();
  }, { timeout: 10000 }).toContain(`${manualGroup.name} → ${manualNode}`);
...
test('后台状态变化后 UI 自动同步并保留已展开手动组', async ({ page, request }) => {
  ...
  await expect.poll(async () => {
    ...
    return page.getByTestId('route-label').textContent();
  }, { timeout: 10000 }).toContain(`${manualGroup.name} → ${nextNode}`);
  await expect(page.getByTestId(`manual-group-${manualGroup.name}`)).toHaveAttribute('open', '');
...
test('自动刷新后保留已展开手动组和页面滚动位置', async ({ page }) => {
  ...
  await expect(page.getByTestId('route-label')).toContainText(`${manualGroupName} → ${secondNode}`);
  await expect(manualGroup).toHaveAttribute('open', '');
